...
const VagrantVMManager = {
  ...
  getSelectedVMs(app) {
    return (
      app.currentProject?.vms?.filter(vm => app.selectedVMs.includes(vm.id)) ||
      []
    )
  },
  ...
  selectVMsByLabel(app, label) {
    ...
    const vmsWithLabel = app.currentProject.vms.filter(
      vm => vm.labels && Array.isArray(vm.labels) && vm.labels.includes(label)
    )
  ...
  getVMCountByLabel(app, label) {
    ...
    return app.currentProject.vms.filter(
      vm => vm.labels && Array.isArray(vm.labels) && vm.labels.includes(label)
    ).length
  },
  ...
  async bulkDeleteVMs(app) {
    ...
    app.setLoading(true)
    try {
      // Delete each VM sequentially to avoid race conditions
      for (const vm of selectedVMs) {
        await api.deleteVM(app.currentProject.id, vm.id)
      }
  ...
  async bulkUpdateVMs(app, updates) {
    ...
    app.setLoading(true)
    try {
      // Update each VM sequentially to avoid race conditions
      for (let vmIndex = 0; vmIndex < selectedVMs.length; vmIndex++) {
        const vm = selectedVMs[vmIndex]
        const vmData = { ...vm }
        ...
        if (
          ...
          updates.network_interfaces.length > 0
        ) {
          ...
          const newInterfaces = updates.network_interfaces.map(iface => {
            ...
            if (sanitized.type === 'private_network') {
              ...
              if (
                ...
                sanitized.ip_assignment === 'static' &&
              ...
              sanitized.host_port = sanitized.host_port
                ? parseInt(sanitized.host_port)
                : null
              sanitized.guest_port = sanitized.guest_port
                ? parseInt(sanitized.guest_port)
                : null
              // Ensure protocol is set (default to tcp)
              if (!sanitized.protocol || sanitized.protocol === '')
        ...
        await api.updateVM(app.currentProject.id, vm.id, vmData)
      }
  ...
  removeEditLabel(app, index) {
    if (
      app.editingVM.labels &&
      index >= 0 &&
      index < app.editingVM.labels.length
    ) {
  ...
  removeBulkLabel(app, index) {
    if (
      app.bulkEditForm.labels &&
      index >= 0 &&
      index < app.bulkEditForm.labels.length
    ) {
