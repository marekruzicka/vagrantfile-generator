...
class PluginService:
    ...
    def _load_plugin_from_file(self, plugin_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if not plugin_file.exists() and self.user_id:
                file_service = FileService()
                shared_path = (
                    file_service.get_shared_data_path("plugins") / f"{plugin_id}.json"
                )
    ...
    def _check_name_conflict(
        self, plugin_name: str, exclude_id: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            if self._check_name_conflict(plugin_data.name):
                raise PluginServiceError(
                    f"Plugin with name '{plugin_data.name}' already exists"
                )
    ...
    def update_plugin(
        self, plugin_id: str, plugin_data: PluginUpdate
    ) -> Optional[Plugin]:
        """
        ...
        try:
            ...
            # This allows explicitly setting fields to None (like clearing default_version)
    ...
    def list_plugins(self, include_deprecated: bool = True) -> List[Plugin]:
        ...
        try:
            ...
            for plugin_data in merged_data:
                ...
                if (
                    plugin.is_shared
                    and not self.show_shared
                    and plugin.id not in favorite_ids
                ):
                    continue
    ...
    def copy_shared_plugin(self, plugin_id: str) -> Plugin:
        ...
        try:
            # Load shared plugin
            shared_file = (
                self.file_service.get_shared_data_path("plugins") / f"{plugin_id}.json"
            )
            ...
            user_file = (
                self.file_service.get_user_data_path(self.user_id, "plugins")
                / f"{new_id}.json"
            )
