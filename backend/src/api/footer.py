...
# Configuration
# Determine footer directory path
# Priority:
# 1. FOOTER_DATA_DIR environment variable
# 2. Relative to this file (development/local)
# 3. Default container path
env_path = os.getenv("FOOTER_DATA_DIR")
if env_path:
    FOOTER_DATA_DIR = env_path
else:
    # Try to find resources relative to this file
    # src/api/footer.py -> src/api -> src -> backend -> resources/footer
    base_dir = Path(__file__).resolve().parent.parent.parent
    local_path = base_dir / "resources" / "footer"

    if local_path.exists():
        FOOTER_DATA_DIR = str(local_path)
    else:
        FOOTER_DATA_DIR = "/app/resources/footer"
...
def get_footer_files() -> Dict[str, List]:
    ...
    return {"files": files, "excluded": excluded, "errors": errors}
...
def parse_markdown_heading(content: str) -> Dict[str, Any]:
    ...
    return {"title": title, "isExternal": is_external, "externalUrl": external_url}
...
async def get_footer_content(filename: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="File contains invalid UTF-8 content"
        )
